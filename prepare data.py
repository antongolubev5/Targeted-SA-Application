...
def check_type_of_entity(entity_pos_start, entity_pos_end, entity, directory_path, file_name):
    ...
    with open(os.path.join(directory_path, file_name + '.ann')) as f:
        for sent in f.readlines():
            ...
            if '\t' in sent:
                if sent.strip().split('\t')[1].split()[1].isdigit() and \
                        sent.strip().split('\t')[1].split()[2].isdigit():
...
def create_tsa_dataset():
    ...
    # для анализа отбираются объекты, для которых есть и .ann и .txt файлы
    dir_path = 'brat/sentiment_dataset'
    files = list(sorted(set([f for f, cnt in Counter([f[:-4] for f in os.listdir(dir_path)]).items() if cnt == 2])))
    # files = ['04_12_12g', '5807_text']
    # files = ['04_12_12g']
    ...
    for file in tqdm(files):
        if os.path.isfile(os.path.join(dir_path, file + '.ann')) and os.path.isfile(
                os.path.join(dir_path, file + '.txt')):

            # анализ разметки .ann для каждого файла
            with open(os.path.join(dir_path, file + '.ann')) as f:
                for sent in f.readlines():
                    ...
                    if 'POSITIVE_TO' in sent or 'POSITIVE_NEG' in sent:
                        entity_info = search_entity(dir_path, file, sent.split()[3].split(':')[1])
                        # проверка принадлежности к списку тэгов
                    ...
                    if 'OPINION_RELATES_TO' in sent:
                        arg1_info = search_entity(dir_path, file, sent.split()[-2].split(':')[1])
                        arg2_info = search_entity(dir_path, file, sent.split()[-1].split(':')[1])
            ...
            if len(source) > 0:
                with open(os.path.join(dir_path, file + '.txt')) as f:
                    # sentencizing
                    f_total_text = f.read()
                    ...
                    # в качестве координат сущности в тексте берется его середина
                    for i in range(len(entity)):
                        entity_pos = (entity_pos_end[i] + entity_pos_start[i]) // 2
                        for j in range(len(sentencized_file)):
                            if sentence_pos_start[j] < entity_pos < sentence_pos_end[j] and entity[i].upper() in sentencized_file[j].upper():
    ...
    out_df = out_df.sort_values(by=['source', 'entity_tag'])
    out_df.to_csv('data/tsa_dataset.csv', sep='\t', index=False)
    print(Counter(out_df['source']))
